...
|-------|----------------|----------|
| [`quickstart.ipynb`](quickstart.ipynb) | Installing the package, the `Input + Config → run → Output` contract, and running a tool | 1 GPU |
...
| [`cloud_inference.ipynb`](cloud_inference.ipynb) | Deploying tools to a Modal workspace you own, then calling them with `device="modal"` | None (compute is remote) |
