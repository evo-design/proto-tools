...
- **The 3Di-extraction binary auto-installs at setup.** Setup downloads a pinned release and uses it for `structureto3didescriptor` 3Di extraction; the GPU build is selected automatically on Linux x86_64 hosts with a compatible NVIDIA driver (`>= 525.60.13`), otherwise a CPU build is used.
