...
| `relax_config` | `PyRosettaRelaxConfig` | `PyRosettaRelaxConfig()` (factory) | Settings used when `pre_relax_structures=True`. Shown in the UI only when the toggle is on (`depends_on={"pre_relax_structures": [True]}`). |
...
- **Seed-sensitive stochastic protocols.** `pyrosetta-relax` and `pyrosetta-interface-analyzer` depend on Rosetta's process-global RNG. When `config.seed` is set on a multi-input call, the tool wrapper dispatches each input separately with a deterministic per-item-derived seed. When `config.seed is None`, repeated calls skip cache lookup/storage and iterable dedup so the RNG can advance naturally.
