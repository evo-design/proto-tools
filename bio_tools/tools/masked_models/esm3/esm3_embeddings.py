...
from bio_tools.tools.masked_models.shared_data_models import (
    MaskedModelConfig,
    MaskedModelInput,
    MaskedModelOutput,
)
...
def run_esm3_embeddings(inputs: ESM3EmbeddingsInput, config: ESM3EmbeddingsConfig) -> ESM3EmbeddingsOutput:
    ...
    local GPU execution and distributed Modal GPU execution.
    ...
    if use_modal_gpu():
        # Modal
        logger.debug(f"Using Modal for ESM3 inference: {config.model_checkpoint}")
        import modal

        ESM3Service = modal.Cls.from_name("bio-programming", "ESM3Service")
