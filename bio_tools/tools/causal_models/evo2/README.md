...
## Overview
Evo2 is Arc Institute's DNA language model for genomic sequence generation and scoring. It performs autoregressive generation of DNA from prompts and can score sequences by log-likelihood. The tool supports local GPU/CPU execution and optional Modal GPU execution, with KV caching for efficient long generations in local mode.
...
Modules in `bio_tools.tools.causal_models.evo2`:
...
- **Modal execution** is used automatically when configured, but does **not** support KV cache reuse.
...
```python
from bio_tools.tools.causal_models.evo2 import (
