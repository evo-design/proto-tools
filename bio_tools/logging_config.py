"""
Logging configuration for bio_tools.
...
class BioToolsOnlyFilter(logging.Filter):
    """Filter to only allow logs from bio_tools project packages."""
    def filter(self, record):
        # Include logs from bio_tools and tests
        allowed_prefixes = ("bio_tools", "tests")
...
def setup_logging(
    ...
    """
    Configure logging for bio_tools.
    ...
    Args:
        ...
        log_dir: Directory for log files. Defaults to logs/ in project root
            or BIO_TOOLS_LOG_DIR environment variable.
        log_filename: Custom filename for the log file. If None, uses timestamped filename
            like bio_tools_YYYYMMDD_HHMMSS.log. If provided, uses this exact filename.
    ...
    # Configure root logger for bio_tools
    root_logger = logging.getLogger("bio_tools")
    ...
    if log_to_file:
        if log_filename is None:
            ...
            log_file = log_path / f"bio_tools_{timestamp}.log"
        else:
        ...
        # Add filter to only log bio_tools messages to file
...
def get_logger(name: str) -> logging.Logger:
    """
    Get a logger for a bio_tools module.

    Args:
        name: Logger name, typically __name__ from the calling module.
            Will be prefixed with 'bio_tools.' if not already.
    ...
    """
    if not name.startswith("bio_tools"):
        name = f"bio_tools.{name}"
