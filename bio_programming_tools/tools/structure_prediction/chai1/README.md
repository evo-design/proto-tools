...
Chai1 requires GPU with >=40GB VRAM (H100 recommended; can run on A100-80GB). It supports Modal cloud deployment for high-throughput or when local GPU is unavailable.
...
6. **Using local mode on Modal:** Modal execution requires `colabfold_search_config.search_mode="remote"` when `use_msa=True`.
