...
- **Execution**: GPU required (Modal cloud or local venv)
...
|------|-----------|---------|--------|
| Modal (cloud GPU) | `use_modal_gpu() == True` | `LigandMPNNService` on Modal | Remote GPU |
| Local venv | `use_modal_gpu() == False` | `EnvManager("ligandmpnn")` running `standalone/inference.py` | Local GPU (`cuda`) |

Both modes produce identical results. Modal is used in production deployments; local venv is used for development and testing.
...
- **GPU is required.** LigandMPNN runs on GPU via Modal or local CUDA. CPU execution is technically possible but impractically slow for typical batch sizes.
