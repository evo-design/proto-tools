...
Evo2 is Arc Institute's genome-scale DNA language model for sequence generation and scoring. Trained on billions of nucleotides spanning prokaryotic and eukaryotic genomes, Evo2 performs autoregressive generation of DNA from prompts and scores sequences by log-likelihood. The tool supports local GPU/CPU execution and optional Modal GPU execution, with KV caching for efficient long generations in local mode.
...
- **Modal execution**: Automatically used when Modal GPU is configured. Same API but does **not** support KV cache reuse across calls.
...
- **KV caching**: Use `cached_generation=True` (default) for long sequences in local mode. KV caching stores intermediate attention states to avoid recomputation, but is **not available in Modal execution mode**.
