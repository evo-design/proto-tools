...
The execution mode is selected automatically based on the `USE_MODAL_GPU` environment variable. For ensemble mode, Modal runs all 4 replicates server-side in one call; local mode runs them sequentially.
