...
def test_parade_hf_token_only_sent_to_real_hf_hosts(monkeypatch, tmp_path) -> None:
    """HF_TOKEN attaches only for huggingface.co and its subdomains, never look-alike hosts."""
    import sys
    from pathlib import Path
    ...
    try:
        ...
        class _FakeOpener:
            def open(self, request, timeout=0):  # a stand-in for the urllib opener
                captured["auth"] = request.get_header("Authorization")
                return _FakeResponse()

        monkeypatch.setattr(inference.urllib.request, "build_opener", lambda *a, **k: _FakeOpener())
        monkeypatch.setattr(inference, "_md5", lambda _path: "")  # expected_md5="" => no checksum
        ...
        # Look-alike hosts must NOT receive the token.
        ...
        # Real host and real subdomains over HTTPS do.
        ...
        # Plaintext HTTP is refused outright — the checkpoint is unpickled, never over HTTP.
        with pytest.raises(ValueError, match="https"):
            inference._download_checkpoint("http://huggingface.co/x.ckpt", "", "plain.ckpt", tmp_path)
    finally:
        sys.path.remove(str(standalone))
        for mod in ("inference", "parade_scoring", "standalone_helpers"):
            sys.modules.pop(mod, None)


def test_parade_redirect_aborts_downgrade_and_strips_offhost_auth() -> None:
    ...
    try:
        ...
        def redirect_to(newurl: str) -> str | None:
            req = ur.Request("https://huggingface.co/x.ckpt")
        ...
        # Off-host HTTPS redirect: allowed to proceed, but the token is stripped.
        assert redirect_to("https://attacker.example/x.ckpt") is None
        # HTTPS->HTTP downgrade: refused outright (a MITM on the plaintext hop could swap the pickle).
        with pytest.raises(urllib.error.HTTPError):
            redirect_to("http://huggingface.co/x.ckpt")
        # A hop to another real HF HTTPS host keeps the token.
...
def test_parade_download_lock_is_mutually_exclusive_and_auto_releases(monkeypatch, tmp_path) -> None:
    ...
    try:
        ...
        # Sequential acquire/release works, and the lock file is never unlinked (no inode race).
        ...
        # While a separate fd holds an exclusive flock, acquisition cannot succeed (times out).
        blocker = inference.os.open(str(lock_path), inference.os.O_CREAT | inference.os.O_RDWR)
        try:
            fcntl.flock(blocker, fcntl.LOCK_EX)
            monkeypatch.setattr(inference, "LOCK_TIMEOUT_SECONDS", 0)
            with pytest.raises(TimeoutError):
                with inference._file_lock(lock_path):
                    pass
        finally:
            fcntl.flock(blocker, fcntl.LOCK_UN)
            inference.os.close(blocker)

        # Once the holder releases (the kernel does this automatically on process exit too),
        # acquisition succeeds again — no stale lock to reclaim.
        monkeypatch.setattr(inference, "LOCK_TIMEOUT_SECONDS", 600)
        with inference._file_lock(lock_path):
            assert lock_path.exists()
    finally:
        for mod in ("inference", "parade_scoring", "standalone_helpers"):
...
def test_parade_redact_url_strips_credentials_and_query() -> None:
    ...
    try:
        ...
        assert "host.co:8443" in redacted  # host:port preserved for debuggability
        assert inference._redact_url("https://[::1]:8443/x.ckpt") == "https://[::1]:8443/<redacted-path>"
        # A malformed port must not crash the redactor (defense-in-depth for logging/errors).
...
def test_parade_checkpoint_url_path_secret_is_redacted_from_log_and_error(monkeypatch, tmp_path, caplog) -> None:
    """Path-carried URL credentials never appear in download logs or checksum errors."""
    import sys
    from pathlib import Path
...
def test_parade_download_does_not_follow_predictable_temp_symlink(monkeypatch, tmp_path) -> None:
...
def test_parade_download_does_not_accept_destination_symlink(monkeypatch, tmp_path) -> None:
    ...
    try:
        ...
        class _FakeResponse:
            ...
            def read(self, _size: int) -> bytes:
                if getattr(self, "_done", False):
                    return b""
                self._done = True
                return b"downloaded-checkpoint"
        ...
        downloaded = inference._download_checkpoint("https://example.com/model.ckpt", "", destination.name, tmp_path)
...
def test_parade_download_cleans_temp_file_on_interrupt(monkeypatch, tmp_path) -> None:
...
def test_parade_checkpoint_filename_cannot_escape_cache(tmp_path) -> None:
    """Malformed raw worker payloads cannot use checkpoint filenames for path traversal."""
    import sys
    from pathlib import Path
