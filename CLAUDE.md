...
- `qc_pending=True` indicates a README that has not yet been migrated to the structured template. `sections.tools` is empty in that case (the parser cannot find the canonical `## Tools` H3 layout), and `toolkit_notes` is empty. See [issue #743](https://github.com/proto-bio/proto-tools/issues/743) for migration progress; both fields populate automatically as toolkits migrate.
