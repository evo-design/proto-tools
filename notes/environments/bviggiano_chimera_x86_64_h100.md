...
- **Commit**: `b5cd76a4438a`
- **Branch**: `investigate-chai1-env-failure`
- **Dirty**: Yes
...
CLAUDE_CODE_EXECPATH=/home/bviggiano/.local/share/claude/versions/2.1.110
CLAUDE_CODE_SSE_PORT=46693
COLORTERM=truecolor
...
GIT_EDITOR=true
GK_GL_ADDR=http://127.0.0.1:39699
GK_GL_PATH=/tmp/gitkraken/gitlens/gitlens-ipc-server-3300045-39699.json
HOME=/home/bviggiano
LANG=C.UTF-8
...
VSCODE_IPC_HOOK_CLI=/tmp/vscode-ipc-94f743d7-17fd-42c3-b0c8-783d13b311f8.sock
...
```
CONDA_PREFIX=/large_storage/hielab/bviggiano/proto_cache/proto_tool_envs/chai1_env
...
LOGNAME=bviggiano
PATH=/large_storage/hielab/bviggiano/proto_cache/proto_tool_envs/chai1_env/bin:/usr/local/cuda/bin:/home/bviggiano/miniforge3/envs/proto-tools/bin:/home/bviggiano/miniforge3/condabin:/home/bviggiano/.curso...
...
UV_HTTP_TIMEOUT=300
VIRTUAL_ENV=/large_storage/hielab/bviggiano/proto_cache/proto_tool_envs/chai1_env
XLA_PYTHON_CLIENT_ALLOCATOR=platform
XLA_PYTHON_CLIENT_PREALLOCATE=false
```
...
|------|--------------|----------------------|----------|-----------|--------|
| `evo1-sample` | yes | ✅ | 178.9s | `b5cd76a` | ✅ Pass |
| `evo2-sample` | yes | ✅ | 298.3s | `b5cd76a` | ✅ Pass |
| `progen2-sample` | yes | ✅ | 80.8s | `b5cd76a` | ✅ Pass |
| `progen3-sample` | yes | ✅ | 28.7s | `b5cd76a` | ✅ Pass |

### Gene Annotation (5/5)

| Tool | Requires GPU | Venv Build Succeeded | Duration | Tested At | Status |
|------|--------------|----------------------|----------|-----------|--------|
| `blast-create-db` | no | ✅ | 30.3s | `b5cd76a` | ✅ Pass |
| `crispr-tracr` | no | ✅ | 156.7s | `b5cd76a` | ✅ Pass |
| `minced-crispr` | no | ✅ | 15.1s | `b5cd76a` | ✅ Pass |
| `mmseqs-clustering` | no | ✅ | 19.1s | `b5cd76a` | ✅ Pass |
| `pyhmmer-hmmscan` | no | ✅ | 20.3s | `b5cd76a` | ✅ Pass |

### Inverse Folding (4/4)

| Tool | Requires GPU | Venv Build Succeeded | Duration | Tested At | Status |
|------|--------------|----------------------|----------|-----------|--------|
| `esm-if1-sample` | yes | ✅ | 50.8s | `b5cd76a` | ✅ Pass |
| `fampnn-pack` | yes | ✅ | 114.3s | `b5cd76a` | ✅ Pass |
| `ligandmpnn-sample` | yes | ✅ | 179.6s | `b5cd76a` | ✅ Pass |
| `proteinmpnn-sample` | yes | ✅ | 63.5s | `b5cd76a` | ✅ Pass |

### Masked Models (3/3)

| Tool | Requires GPU | Venv Build Succeeded | Duration | Tested At | Status |
|------|--------------|----------------------|----------|-----------|--------|
| `ablang-embedding` | yes | ✅ | 47.8s | `b5cd76a` | ✅ Pass |
| `esm2-embedding` | yes | ✅ | 50.9s | `b5cd76a` | ✅ Pass |
| `esm3-embedding` | yes | ✅ | 71.2s | `b5cd76a` | ✅ Pass |

### Mutagenesis (2/2)

| Tool | Requires GPU | Venv Build Succeeded | Duration | Tested At | Status |
|------|--------------|----------------------|----------|-----------|--------|
| `random-nucleotide-sample` | no | - | 0.0s | `b5cd76a` | ✅ Pass |
| `random-protein-sample` | no | - | 0.0s | `b5cd76a` | ✅ Pass |

### Orf Prediction (2/2)

| Tool | Requires GPU | Venv Build Succeeded | Duration | Tested At | Status |
|------|--------------|----------------------|----------|-----------|--------|
| `orfipy-prediction` | no | ✅ | 16.3s | `b5cd76a` | ✅ Pass |
| `prodigal-prediction` | no | ✅ | 14.8s | `b5cd76a` | ✅ Pass |

### Rna Splicing (1/1)

| Tool | Requires GPU | Venv Build Succeeded | Duration | Tested At | Status |
|------|--------------|----------------------|----------|-----------|--------|
| `splice-transformer-prediction` | yes | ✅ | 35.7s | `b5cd76a` | ✅ Pass |

### Sequence Alignment (2/2)

| Tool | Requires GPU | Venv Build Succeeded | Duration | Tested At | Status |
|------|--------------|----------------------|----------|-----------|--------|
| `colabfold-search` | no | ✅ | 38.3s | `b5cd76a` | ✅ Pass |
| `mafft-align` | no | ✅ | 20.2s | `b5cd76a` | ✅ Pass |

### Sequence Scoring (4/4)

| Tool | Requires GPU | Venv Build Succeeded | Duration | Tested At | Status |
|------|--------------|----------------------|----------|-----------|--------|
| `alphagenome-predict-intervals` | yes | ✅ | 160.6s | `b5cd76a` | ✅ Pass |
| `borzoi-ensemble` | yes | ✅ | 85.5s | `b5cd76a` | ✅ Pass |
| `enformer-prediction` | yes | ✅ | 39.5s | `b5cd76a` | ✅ Pass |
| `segmasker-score` | no | ✅ | 27.1s | `b5cd76a` | ✅ Pass |

### Structure Alignment (2/2)

| Tool | Requires GPU | Venv Build Succeeded | Duration | Tested At | Status |
|------|--------------|----------------------|----------|-----------|--------|
| `tmalign-alignment` | no | ✅ | 24.8s | `b5cd76a` | ✅ Pass |
| `usalign-alignment` | no | ✅ | 37.9s | `b5cd76a` | ✅ Pass |

### Structure Design (1/1)

| Tool | Requires GPU | Venv Build Succeeded | Duration | Tested At | Status |
|------|--------------|----------------------|----------|-----------|--------|
| `rfdiffusion3-design` | yes | ✅ | 124.7s | `b5cd76a` | ✅ Pass |

### Structure Dynamics (1/1)

| Tool | Requires GPU | Venv Build Succeeded | Duration | Tested At | Status |
|------|--------------|----------------------|----------|-----------|--------|
| `bioemu-sample` | yes | ✅ | 115.2s | `b5cd76a` | ✅ Pass |

### Structure Prediction (7/7)

| Tool | Requires GPU | Venv Build Succeeded | Duration | Tested At | Status |
|------|--------------|----------------------|----------|-----------|--------|
| `alphafold2-gradient` | yes | ✅ | 278.1s | `b5cd76a` | ✅ Pass |
| `alphafold3-prediction` | yes | ✅ | 67.4s | `b5cd76a` | ✅ Pass |
| `boltz2-prediction` | yes | ✅ | 89.4s | `b5cd76a` | ✅ Pass |
| `chai1-prediction` | yes | ✅ | 261.1s | `b5cd76a` ✱ | ✅ Pass |
| `esmfold-prediction` | yes | ✅ | 60.6s | `b5cd76a` | ✅ Pass |
| `protenix-prediction` | yes | ✅ | 342.3s | `b5cd76a` | ✅ Pass |
| `viennarna-prediction` | no | ✅ | 20.0s | `b5cd76a` | ✅ Pass |

### Structure Scoring (2/2)

| Tool | Requires GPU | Venv Build Succeeded | Duration | Tested At | Status |
|------|--------------|----------------------|----------|-----------|--------|
| `pyrosetta-energy` | no | ✅ | 64.8s | `b5cd76a` | ✅ Pass |
| `structure-metrics` | no | ✅ | 17.5s | `b5cd76a` | ✅ Pass |

### Testing (3/3)

| Tool | Requires GPU | Venv Build Succeeded | Duration | Tested At | Status |
|------|--------------|----------------------|----------|-----------|--------|
| `mock-cli-multi-gpu-tool-run` | yes | - | - | `b5cd76a` | ⏭️ Skip |
| `mock-cli-tool-run` | yes | ✅ | 21.0s | `b5cd76a` | ✅ Pass |
| `mock-jax-multi-gpu-tool-run` | yes | - | - | `b5cd76a` | ⏭️ Skip |
| `mock-jax-tool-run` | yes | ✅ | 48.1s | `b5cd76a` | ✅ Pass |
| `mock-pytorch-multi-gpu-tool-run` | yes | - | - | `b5cd76a` | ⏭️ Skip |
| `mock-pytorch-tool-run` | yes | ✅ | 86.3s | `b5cd76a` | ✅ Pass |

---
*Generated at 2026-04-16 14:51:30 by `pytest --env-report`*
