...
- **Commit**: `10c69aa14bda`
- **Branch**: `fix-remaining-dispatch-operation-keys`
- **Dirty**: Yes
...
CLAUDE_CODE_EXECPATH=/home/bviggiano/.local/share/claude/versions/2.1.109
...
|------|--------------|----------------------|----------|-----------|--------|
| `evo1-sample` | yes | ✅ | 697.4s | `00181bb` ✱ | ✅ Pass |
| `evo2-sample` | yes | ✅ | 555.5s | `00181bb` ✱ | ✅ Pass |
| `progen2-sample` | yes | ✅ | 330.5s | `8e5721a` ✱ | ✅ Pass |
| `progen3-sample` | yes | ✅ | 28.9s | `8e5721a` ✱ | ✅ Pass |

### Gene Annotation (5/5)

| Tool | Requires GPU | Venv Build Succeeded | Duration | Tested At | Status |
|------|--------------|----------------------|----------|-----------|--------|
| `blast-create-db` | no | ✅ | 38.0s | `00181bb` ✱ | ✅ Pass |
| `crispr-tracr` | no | ✅ | 147.6s | `00181bb` ✱ | ✅ Pass |
| `minced-crispr` | no | ✅ | 31.1s | `00181bb` ✱ | ✅ Pass |
| `mmseqs-clustering` | no | ✅ | 39.7s | `00181bb` ✱ | ✅ Pass |
| `pyhmmer-hmmscan` | no | ✅ | 34.7s | `8e5721a` ✱ | ✅ Pass |

### Inverse Folding (4/4)

| Tool | Requires GPU | Venv Build Succeeded | Duration | Tested At | Status |
|------|--------------|----------------------|----------|-----------|--------|
| `esm-if1-sample` | yes | ✅ | 151.8s | `00181bb` ✱ | ✅ Pass |
| `fampnn-pack` | yes | ✅ | 397.8s | `00181bb` ✱ | ✅ Pass |
| `ligandmpnn-sample` | yes | ✅ | 406.9s | `00181bb` ✱ | ✅ Pass |
| `proteinmpnn-sample` | yes | ✅ | 134.0s | `8e5721a` ✱ | ✅ Pass |

### Masked Models (3/3)

| Tool | Requires GPU | Venv Build Succeeded | Duration | Tested At | Status |
|------|--------------|----------------------|----------|-----------|--------|
| `ablang-embedding` | yes | ✅ | 230.5s | `00181bb` ✱ | ✅ Pass |
| `esm2-embedding` | yes | ✅ | 180.0s | `00181bb` ✱ | ✅ Pass |
| `esm3-embedding` | yes | ✅ | 216.1s | `00181bb` ✱ | ✅ Pass |

### Mutagenesis (2/2)

| Tool | Requires GPU | Venv Build Succeeded | Duration | Tested At | Status |
|------|--------------|----------------------|----------|-----------|--------|
| `random-nucleotide-sample` | no | - | 0.0s | `8e5721a` ✱ | ✅ Pass |
| `random-protein-sample` | no | - | 0.0s | `8e5721a` ✱ | ✅ Pass |

### Orf Prediction (2/2)

| Tool | Requires GPU | Venv Build Succeeded | Duration | Tested At | Status |
|------|--------------|----------------------|----------|-----------|--------|
| `orfipy-prediction` | no | ✅ | 41.2s | `8e5721a` ✱ | ✅ Pass |
| `prodigal-prediction` | no | ✅ | 20.7s | `8e5721a` ✱ | ✅ Pass |

### Rna Splicing (1/1)

| Tool | Requires GPU | Venv Build Succeeded | Duration | Tested At | Status |
|------|--------------|----------------------|----------|-----------|--------|
| `splice-transformer-prediction` | yes | ✅ | 104.6s | `10c69aa` ✱ | ✅ Pass |

### Sequence Alignment (2/2)

| Tool | Requires GPU | Venv Build Succeeded | Duration | Tested At | Status |
|------|--------------|----------------------|----------|-----------|--------|
| `colabfold-search` | no | ✅ | 116.2s | `00181bb` ✱ | ✅ Pass |
| `mafft-align` | no | ✅ | 45.7s | `00181bb` ✱ | ✅ Pass |

### Sequence Scoring (4/4)

| Tool | Requires GPU | Venv Build Succeeded | Duration | Tested At | Status |
|------|--------------|----------------------|----------|-----------|--------|
| `alphagenome-predict-intervals` | yes | ✅ | 303.3s | `00181bb` ✱ | ✅ Pass |
| `borzoi-ensemble` | yes | ✅ | 158.1s | `00181bb` ✱ | ✅ Pass |
| `enformer-prediction` | yes | ✅ | 132.3s | `00181bb` ✱ | ✅ Pass |
| `segmasker-score` | no | ✅ | 31.9s | `10c69aa` ✱ | ✅ Pass |

### Structure Alignment (2/2)

| Tool | Requires GPU | Venv Build Succeeded | Duration | Tested At | Status |
|------|--------------|----------------------|----------|-----------|--------|
| `tmalign-alignment` | no | ✅ | 26.4s | `10c69aa` ✱ | ✅ Pass |
| `usalign-alignment` | no | ✅ | 38.3s | `10c69aa` ✱ | ✅ Pass |

### Structure Design (1/1)

| Tool | Requires GPU | Venv Build Succeeded | Duration | Tested At | Status |
|------|--------------|----------------------|----------|-----------|--------|
| `rfdiffusion3-design` | yes | ✅ | 302.3s | `10c69aa` ✱ | ✅ Pass |

### Structure Dynamics (1/1)

| Tool | Requires GPU | Venv Build Succeeded | Duration | Tested At | Status |
|------|--------------|----------------------|----------|-----------|--------|
| `bioemu-sample` | yes | ✅ | 281.6s | `00181bb` ✱ | ✅ Pass |

### Structure Prediction (7/7)

| Tool | Requires GPU | Venv Build Succeeded | Duration | Tested At | Status |
|------|--------------|----------------------|----------|-----------|--------|
| `alphafold2-gradient` | yes | ✅ | 432.9s | `10c69aa` ✱ | ✅ Pass |
| `alphafold3-prediction` | yes | ✅ | 72.8s | `00181bb` ✱ | ✅ Pass |
| `boltz2-prediction` | yes | ✅ | 176.7s | `00181bb` ✱ | ✅ Pass |
| `chai1-prediction` | yes | ✅ | 420.7s | `00181bb` ✱ | ✅ Pass |
| `esmfold-prediction` | yes | ✅ | 212.4s | `00181bb` ✱ | ✅ Pass |
| `protenix-prediction` | yes | ✅ | 457.6s | `8e5721a` ✱ | ✅ Pass |
| `viennarna-prediction` | no | ✅ | 15.3s | `10c69aa` ✱ | ✅ Pass |

### Structure Scoring (2/2)

| Tool | Requires GPU | Venv Build Succeeded | Duration | Tested At | Status |
|------|--------------|----------------------|----------|-----------|--------|
| `pyrosetta-energy` | no | ✅ | 68.0s | `8e5721a` ✱ | ✅ Pass |
| `structure-metrics` | no | ✅ | 26.5s | `10c69aa` ✱ | ✅ Pass |

### Testing (3/3)

| Tool | Requires GPU | Venv Build Succeeded | Duration | Tested At | Status |
|------|--------------|----------------------|----------|-----------|--------|
| `mock-cli-multi-gpu-tool-run` | yes | - | - | `00181bb` ✱ | ⏭️ Skip |
| `mock-cli-tool-run` | yes | ✅ | 18.6s | `00181bb` ✱ | ✅ Pass |
| `mock-jax-multi-gpu-tool-run` | yes | - | - | `00181bb` ✱ | ⏭️ Skip |
| `mock-jax-tool-run` | yes | ✅ | 83.8s | `00181bb` ✱ | ✅ Pass |
| `mock-pytorch-multi-gpu-tool-run` | yes | - | - | `00181bb` ✱ | ⏭️ Skip |
| `mock-pytorch-tool-run` | yes | ✅ | 257.9s | `8e5721a` ✱ | ✅ Pass |

---
*Generated at 2026-04-15 12:46:45 by `pytest --env-report`*
