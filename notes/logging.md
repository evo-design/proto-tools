...
[ standalone subprocess ]
    │
    │ records emitted via standalone_helpers.get_logger ──► _BridgeHandler
    │ (loggers under the "worker.*" namespace)               │
    │                                                        ▼
    │                                              "\x00LOG\x00<json>\n"
    │                                                  (tagged stream)
    │
    │ third-party Python loggers (transformers, urllib3), direct print(),
    │ C/C++ writes (PyRosetta std::cout, JAX XLA compile, tqdm progress bars)
    │   ───►  raw stderr bytes (untagged stream)
    │
    ▼ stderr (PIPE)
[ parent: PersistentWorker._drain_stderr / ToolInstance._run_oneshot ]
    ├─ tagged → strip "worker." prefix → re-emit on
    │           logging.getLogger("proto_tools.worker.{toolkit}.{name}")
    │           Standard handlers fire, including SpinnerFromLogsHandler for
    │           records flagged with update_status=True.
    │
    └─ untagged → ring buffer (always; consulted by _crash_context())
                → tee to parent's sys.stderr only when verbose level >= 3
```
...
`BaseConfig.verbose: int` is a 0/1/2/3 scale; `PROTO_WORKER_VERBOSE` env var matches the same scale and acts as a global ceiling override (`max(config.verbose, env)`).
...
```
tqdm (HF, transformers, datasets) writes "\r<frame1>\r<frame2>...<final>\n"
   │
   ▼ untagged stderr
[ _drain_stderr → _handle_raw_line ]
   │
   ├─ at level 0/1/2: not teed; the line lands in the ring buffer (final frame only)
   └─ at level 3: line.rsplit("\r", 1)[-1] keeps only the final frame, then teed
```

Net behavior: at level 3, you see the start banner and "100% complete" of each progress bar; intermediate frames are dropped. Newline-separated progress libraries (rich, alive-progress) are not filtered and produce one line per frame at level 3; this is acceptable noise since the user opted in.

A followup PR will replace last-frame retention with active 0/25/50/75/100% milestone detection that re-emits each milestone via the tagged channel as a `proto_tools.worker.{toolkit}.progress` record. Useful for diagnosing stalls.
...
| `_drain_stderr` / `_handle_raw_line` / `_reemit_tagged` | `proto_tools/utils/persistent_worker.py` | Drain thread; demuxes tagged lines, ring-buffers / tees untagged |
...
- The producer side has **zero `proto_tools` imports**. The full bridge implementation ships via `standalone_helpers/proto_logging.py` (stdlib-only). This was a deliberate redesign — the previous `from proto_tools.utils.logging_config import install_subprocess_bridge` path failed silently in every tool venv because `proto_tools` (and `pydantic`) aren't installed inside the venv. See PR #737 follow-up for context.
