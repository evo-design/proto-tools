...
<Tabs>
  <Tab title="PDB Fetch Entry">
  **Key:** `pdb-fetch-entry`

  Fetch structure metadata (title, method, resolution) from RCSB PDB

### Input

<ParamField path="pdb_id" type="string" required>
...
<ParamField path="http_retries" type="integer" default="3">
  Max HTTP retries
</ParamField>
...
<ParamField path="user_agent" type="string" default="bio-programming-tools/pdb-fetch-v1">
...
<ResponseField name="source_url" type="string">
  ...
  <Tab title="PDB Fetch FASTA">
  **Key:** `pdb-fetch-fasta`

  Fetch chain sequences from RCSB PDB with protein/nucleotide classification
...
<ResponseField name="chains" type="List[PdbChain]">
  Parsed chain sequences
</ResponseField>
