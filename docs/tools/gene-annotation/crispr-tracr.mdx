...
description: "CRISPRtracrRNA predicts tracrRNA (trans-activating CRISPR RNA) sequences from nucleotide CRISPR loci using covariance models and RNA-RNA interaction prediction. It identifies tracrRNA candidates, scores them by E-value and anti-repeat similarity, and predicts the anti-repeat interaction structure via IntaRNA."
---
...
<ParamField path="sequences" type="List[string]" required>
  Nucleotide sequence(s) to predict tracrRNA from
</ParamField>
...
<ParamField path="model_type" type="enum" default="II">
  CRISPR model type: "II" for type II only (faster), "all" for comprehensive

  Options: `II`, `all`
</ParamField>

<ParamField path="run_type" type="enum" default="complete_run">
  Pipeline mode: "complete_run" for full analysis, "model_only" for fast scan

  Options: `complete_run`, `model_only`
</ParamField>

<ParamField path="num_workers" type="integer">
  Number of parallel workers (defaults to SLURM CPUs or 1)
</ParamField>

### Output

<ResponseField name="predictions" type="List[TracrPrediction]">
  Per-sequence tracrRNA predictions
</ResponseField>
