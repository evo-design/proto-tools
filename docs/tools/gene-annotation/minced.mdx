---
title: "MinCED"
description: "MinCED (Mining CRISPRs in Environmental Datasets) is a tool for detecting CRISPR arrays in nucleotide sequences. It identifies repeat-spacer arrays using a heuristic k-mer search algorithm derived from the CRISPR Recognition Tool (CRT), returning structured information about each array's repeats, spacers, and their genomic positions."
---
...
<ParamField path="sequences" type="List[string]" required>
  Nucleotide sequence(s) to search for CRISPR arrays
</ParamField>
...
<ParamField path="min_num_repeats" type="integer" default="3">
  Minimum number of repeats required for a CRISPR array
</ParamField>

<ParamField path="min_repeat_length" type="integer" default="27">
  Minimum length of a repeat sequence in nucleotides
</ParamField>

### Output

<ResponseField name="results" type="List[MincedSequenceResult]">
  Per-sequence CRISPR array detection results
</ResponseField>
