...
description: "SpliceTransformer is a deep learning model for predicting splice sites with tissue-specific resolution. It identifies splice donors (5' splice sites) and acceptors (3' splice sites) across 15 human tissues, enabling analysis of alternative splicing patterns and tissue-specific isoform usage. The model uses transformer architecture with long-range sequence context for accurate splice site prediction."
---
...
<ParamField path="target_seqs" type="List[string]" required>
  Sequence(s) on which to make splicing predictions
</ParamField>

<ParamField path="left_contexts" type="List[string]" required>
  Sequence(s) of the left context. Must be the same length as target_seqs
</ParamField>

<ParamField path="right_contexts" type="List[string]" required>
  Sequence(s) of the right context. Must be the same length as target_seqs
</ParamField>

### Configuration

<ParamField path="context_length" type="integer" default="4000">
  Context length on both left and right of target sequence.
</ParamField>

### Output

<ResponseField name="prediction" type="ndarray" required>
  Matrix of (batch, target_length, 18)
</ResponseField>
